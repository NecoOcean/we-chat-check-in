...
CREATE TABLE `activities` (
  ...
  `scope_county_code` VARCHAR(16) NULL COMMENT '县域范围（县级活动必填，市级可空或指定县）',
  `created_by` BIGINT NOT NULL COMMENT '发起管理员',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `ended_at` DATETIME NULL COMMENT '结束时间（操作时间）',
  PRIMARY KEY (`id`),
  ...
  CONSTRAINT `fk_activities_created_by` FOREIGN KEY (`created_by`) REFERENCES `admins` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
...
CREATE TABLE `qrcodes` (
  ...
  `status` ENUM('active', 'disabled', 'expired') NOT NULL DEFAULT 'active' COMMENT '状态：正常/禁用/过期',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  ...
  INDEX `idx_qrcodes_expire_at` (`expire_at`),
...
CREATE TABLE `checkins` (
  ...
  INDEX `idx_checkins_activity_submitted` (`activity_id`, `submitted_at`),
...
CREATE TABLE `evaluations` (
  ...
  INDEX `idx_evaluations_activity_submitted` (`activity_id`, `submitted_at`),
...
CREATE TABLE `audit_logs` (
  ...
  PRIMARY KEY (`id`),
  INDEX `idx_audit_logs_actor_created` (`actor_admin_id`, `created_at`),
